...
void test_create_small_ring_buffer(void) {
    ...
    RingBuffer buffer;
    CU_ASSERT_EQUAL(create_ring_buffer(sizeof(int), &buffer), RBUFFER_OK);
    
    for (int i = 0; i < 5; i++) {
        CU_ASSERT_EQUAL(ring_buffer_insert(&buffer, &i), RBUFFER_OK);
    }
...
void test_ring_buffer_insert_and_remove_first(void) {
    ...
    int value = -1;
    for (int i = 0; i < 10; i++) {
        CU_ASSERT_EQUAL(get_and_remove_first(&buffer, &value), RBUFFER_OK);
...
void test_ring_buffer_insert_and_remove_last(void) {
    ...
    for (int i = 9; i >= 0; i--) {
        CU_ASSERT_EQUAL(get_and_remove_last(&buffer, &value), RBUFFER_OK);
        CU_ASSERT_EQUAL(value, i);
    }
...
void test_remove_from_empty_ring_buffer(void) {
    ...
    int value = -1;
    CU_ASSERT_EQUAL(get_and_remove_first(&buffer, &value), RBUFFER_ERROR_IS_EMPTY);
    CU_ASSERT_EQUAL(get_and_remove_last(&buffer, &value), RBUFFER_ERROR_IS_EMPTY);
...
void test_invalid_params_ring_buffer(void) {
    ...
    CU_ASSERT_EQUAL(ring_buffer_insert(&buffer, NULL), RBUFFER_ERROR_INVALID_PARAM);
    // Passing NULL buffer to insert.
    CU_ASSERT_EQUAL(ring_buffer_insert(NULL, &value), RBUFFER_ERROR_INVALID_PARAM);
    // Passing NULL out pointer to get_and_remove_first/last.
    CU_ASSERT_EQUAL(get_and_remove_first(&buffer, NULL), RBUFFER_ERROR_INVALID_PARAM);
    CU_ASSERT_EQUAL(get_and_remove_last(&buffer, NULL), RBUFFER_ERROR_INVALID_PARAM);
...
void test_ring_buffer_wrap_around(void) {
    ...
    for (int i = 0; i < 3; i++) {
        CU_ASSERT_EQUAL(ring_buffer_insert(&buffer, &new_vals[i]), RBUFFER_OK);
    }
